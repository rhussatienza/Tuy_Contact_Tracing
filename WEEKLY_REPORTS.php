...
if(isset($_REQUEST['submit'])){
  ...
  $search = $_POST['first_date'];
     ...
     <script type="text/javascript">
            ...
            function(isConfirm){
              if (isConfirm) {
            window.location.href = "WEEKLY_REPORTS.php";
              }
            }); }, 5);
    </script>
  <?php
...
$acle = mysqli_query($con, "SELECT COUNT(*) AS acle FROM tbl_people_updated WHERE address = 'Acle' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $ACLE = mysqli_fetch_assoc($acle);
  $bayudbud = mysqli_query($con, "SELECT COUNT(*) AS bayudbud FROM tbl_people_updated WHERE address = 'Bayudbud' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $BAYUDBUD = mysqli_fetch_assoc($bayudbud);
  $bolboc = mysqli_query($con, "SELECT COUNT(*) AS bolboc FROM tbl_people_updated WHERE address = 'Bolboc' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $BOLBOC = mysqli_fetch_assoc($bolboc);
  $dalima= mysqli_query($con, "SELECT COUNT(*) AS dalima FROM tbl_people_updated WHERE address = 'Dalima' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $DALIMA = mysqli_fetch_assoc($dalima);
  $dao= mysqli_query($con, "SELECT COUNT(*) AS dao FROM tbl_people_updated WHERE address = 'Dao' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $DAO = mysqli_fetch_assoc($dao);
  $guinhawa= mysqli_query($con, "SELECT COUNT(*) AS guinhawa FROM tbl_people_updated WHERE address = 'Guinhawa' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $GUINHAWA = mysqli_fetch_assoc($guinhawa);
  $lumbangan= mysqli_query($con, "SELECT COUNT(*) AS lumbangan FROM tbl_people_updated WHERE address = 'Lumbangan' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $LUMBANGAN = mysqli_fetch_assoc($lumbangan);
  $luntal = mysqli_query($con, "SELECT COUNT(*) AS luntal FROM tbl_people_updated WHERE address = 'Luntal' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $LUNTAL = mysqli_fetch_assoc($luntal);
  $magahis = mysqli_query($con, "SELECT COUNT(*) AS magahis FROM tbl_people_updated WHERE address = 'Magahis' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $MAGAHIS = mysqli_fetch_assoc($magahis);
  $malibu = mysqli_query($con, "SELECT COUNT(*) AS malibu FROM tbl_people_updated WHERE address = 'Malibu' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $MALIBU = mysqli_fetch_assoc($malibu);
  $mataywanac = mysqli_query($con, "SELECT COUNT(*) AS mataywanac FROM tbl_people_updated WHERE address = 'Mataywanac' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $MATAYWANAC = mysqli_fetch_assoc($mataywanac);
  $palincaro = mysqli_query($con, "SELECT COUNT(*) AS palincaro FROM tbl_people_updated WHERE address = 'Palincaro' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $PALINCARO = mysqli_fetch_assoc($palincaro);
  $luna = mysqli_query($con, "SELECT COUNT(*) AS luna FROM tbl_people_updated WHERE address = 'Luna' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $LUNA = mysqli_fetch_assoc($luna);
  $burgos = mysqli_query($con, "SELECT COUNT(*) AS burgos FROM tbl_people_updated WHERE address = 'Burgos' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $BURGOS = mysqli_fetch_assoc($burgos);
  $rizal = mysqli_query($con, "SELECT COUNT(*) AS rizal FROM tbl_people_updated WHERE address = 'Rizal' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $RIZAL = mysqli_fetch_assoc($rizal);
  $rillo = mysqli_query($con, "SELECT COUNT(*) AS rillo FROM tbl_people_updated WHERE address = 'Rillo' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $RILLO = mysqli_fetch_assoc($rillo);
  $putol = mysqli_query($con, "SELECT COUNT(*) AS putol FROM tbl_people_updated WHERE address = 'Putol' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $PUTOL = mysqli_fetch_assoc($putol);
  $sabang = mysqli_query($con, "SELECT COUNT(*) AS sabang FROM tbl_people_updated WHERE address = 'Sabang' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $SABANG = mysqli_fetch_assoc($sabang);
  $sanjose = mysqli_query($con, "SELECT COUNT(*) AS sanjose FROM tbl_people_updated WHERE address = 'San Jose' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $SANJOSE = mysqli_fetch_assoc($sanjose);
  $talon = mysqli_query($con, "SELECT COUNT(*) AS talon FROM tbl_people_updated WHERE address = 'Talon' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $TALON = mysqli_fetch_assoc($talon);
  $toong = mysqli_query($con, "SELECT COUNT(*) AS toong FROM tbl_people_updated WHERE address = 'Toong' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $TOONG = mysqli_fetch_assoc($toong);
  $tuyon = mysqli_query($con, "SELECT COUNT(*) AS tuyon FROM tbl_people_updated WHERE address = 'Tuyon-Tuyon' AND status = 'Successful Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $TUYON = mysqli_fetch_assoc($tuyon);

  $cancelled = mysqli_query($con, "SELECT COUNT(*) AS cancelled FROM tbl_people_updated WHERE status = 'Cancelled Appointment' AND a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $CANCELLED = mysqli_fetch_assoc($cancelled);

  $S1 =mysqli_query($con, "SELECT COUNT(*) AS num FROM tbl_people WHERE a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
  $ids = mysqli_fetch_assoc($S1);
  $S2 =mysqli_query($con, "SELECT COUNT(*) AS num FROM tbl_people_updated WHERE a_date >= '$first' AND a_date <= '$first' + INTERVAL 5 DAY");
...
<body>
  ...
  <div class="row d-flex justify-content-center">
    ...
    <h1>Municipality of Tuy Batangas</h1>
        <div class="container">
          <div class="title">Residents Weekly Reports</div>
           <div class="content">
            <form method="POST">
...
error_reporting(E_ALL ^ E_WARNING); 
   ...
   <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span>
    <center>Please Enter Date!</center>
     ...
     $insertQuery = mysqli_query($con, "INSERT INTO tbl_brgy_weekly VALUES ('$ACLE1', '$BAYUDBUD1', '$BOLBOC1', '$DALIMA1', '$DAO1', '$GUINHAWA1', '$LUMBANGAN1', '$LUNTAL1', '$MAGAHIS1', '$MALIBU1', '$MATAYWANAC1',   '$PALINCARO1', '$LUNA1', '$BURGOS1', '$RIZAL1', '$RILLO1', '$PUTOL1', '$SABANG1', '$SANJOSE1', '$TALON1', '$TOONG1', '$TUYON1', '$CANCELLED', '$MISSED','$DATE1')");?>
...
confirmButtonText: "Okay"
      ...
      window.location = "WEEKLY_REPORTS.php";
  </script>
...
<button type="submit" name="enter" class="btn btn-success"onclick="return confirm('Are you sure that do you want to save this data?');">Save Data</button></div>
                  ...
                  <form action = "LIST_REPORTS_WEEKLY.php">
