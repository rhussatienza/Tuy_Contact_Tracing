...
if(isset($_REQUEST['submit'])){
  ...
  $search = $_POST['a_date'];
     ...
     <script type="text/javascript">
            ...
            function(isConfirm){
              if (isConfirm) {
            window.location.href = "CREATE_REPORTS.php";
              }
            }); }, 5);
    </script>
  <?php
...
confirmButtonText: "Okay"
    ...
    <?php
$acle = mysqli_query($con, "SELECT COUNT(*) AS acle FROM tbl_people_updated WHERE address = 'Acle' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $ACLE = mysqli_fetch_assoc($acle);
  $bayudbud = mysqli_query($con, "SELECT COUNT(*) AS bayudbud FROM tbl_people_updated WHERE address = 'Bayudbud' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $BAYUDBUD = mysqli_fetch_assoc($bayudbud);
  $bolboc = mysqli_query($con, "SELECT COUNT(*) AS bolboc FROM tbl_people_updated WHERE address = 'Bolboc' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $BOLBOC = mysqli_fetch_assoc($bolboc);
  $dalima= mysqli_query($con, "SELECT COUNT(*) AS dalima FROM tbl_people_updated WHERE address = 'Dalima' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $DALIMA = mysqli_fetch_assoc($dalima);
  $dao= mysqli_query($con, "SELECT COUNT(*) AS dao FROM tbl_people_updated WHERE address = 'Dao' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $DAO = mysqli_fetch_assoc($dao);
  $guinhawa= mysqli_query($con, "SELECT COUNT(*) AS guinhawa FROM tbl_people_updated WHERE address = 'Guinhawa' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $GUINHAWA = mysqli_fetch_assoc($guinhawa);
  $lumbangan= mysqli_query($con, "SELECT COUNT(*) AS lumbangan FROM tbl_people_updated WHERE address = 'Lumbangan' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $LUMBANGAN = mysqli_fetch_assoc($lumbangan);
  $luntal = mysqli_query($con, "SELECT COUNT(*) AS luntal FROM tbl_people_updated WHERE address = 'Luntal' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $LUNTAL = mysqli_fetch_assoc($luntal);
  $magahis = mysqli_query($con, "SELECT COUNT(*) AS magahis FROM tbl_people_updated WHERE address = 'Magahis' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $MAGAHIS = mysqli_fetch_assoc($magahis);
  $malibu = mysqli_query($con, "SELECT COUNT(*) AS malibu FROM tbl_people_updated WHERE address = 'Malibu' AND status = 'Successful Appointment' AND  a_date like '%$search%'");
  $MALIBU = mysqli_fetch_assoc($malibu);
  $mataywanac = mysqli_query($con, "SELECT COUNT(*) AS mataywanac FROM tbl_people_updated WHERE address = 'Mataywanac' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $MATAYWANAC = mysqli_fetch_assoc($mataywanac);
  $palincaro = mysqli_query($con, "SELECT COUNT(*) AS palincaro FROM tbl_people_updated WHERE address = 'Palincaro' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $PALINCARO = mysqli_fetch_assoc($palincaro);
  $luna = mysqli_query($con, "SELECT COUNT(*) AS luna FROM tbl_people_updated WHERE address = 'Luna' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $LUNA = mysqli_fetch_assoc($luna);
  $burgos = mysqli_query($con, "SELECT COUNT(*) AS burgos FROM tbl_people_updated WHERE address = 'Burgos' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $BURGOS = mysqli_fetch_assoc($burgos);
  $rizal = mysqli_query($con, "SELECT COUNT(*) AS rizal FROM tbl_people_updated WHERE address = 'Rizal' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $RIZAL = mysqli_fetch_assoc($rizal);
  $rillo = mysqli_query($con, "SELECT COUNT(*) AS rillo FROM tbl_people_updated WHERE address = 'Rillo' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $RILLO = mysqli_fetch_assoc($rillo);
  $putol = mysqli_query($con, "SELECT COUNT(*) AS putol FROM tbl_people_updated WHERE address = 'Putol' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $PUTOL = mysqli_fetch_assoc($putol);
  $sabang = mysqli_query($con, "SELECT COUNT(*) AS sabang FROM tbl_people_updated WHERE address = 'Sabang' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $SABANG = mysqli_fetch_assoc($sabang);
  $sanjose = mysqli_query($con, "SELECT COUNT(*) AS sanjose FROM tbl_people_updated WHERE address = 'San Jose' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $SANJOSE = mysqli_fetch_assoc($sanjose);
  $talon = mysqli_query($con, "SELECT COUNT(*) AS talon FROM tbl_people_updated WHERE address = 'Talon' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $TALON = mysqli_fetch_assoc($talon);
  $toong = mysqli_query($con, "SELECT COUNT(*) AS toong FROM tbl_people_updated WHERE address = 'Toong' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $TOONG = mysqli_fetch_assoc($toong);
  $tuyon = mysqli_query($con, "SELECT COUNT(*) AS tuyon FROM tbl_people_updated WHERE address = 'Tuyon-Tuyon' AND status = 'Successful Appointment' AND a_date like '%$search%'");
  $TUYON = mysqli_fetch_assoc($tuyon);

  $cancelled = mysqli_query($con, "SELECT COUNT(*) AS cancelled FROM tbl_people_updated WHERE status = 'Cancelled Appointment' AND a_date like '%$search%'");
  ...
  $S2 =mysqli_query($con, "SELECT COUNT(*) AS num FROM tbl_people_updated WHERE a_date LIKE '%$search%'");
...
<body>
  ...
  <div class="row d-flex justify-content-center">
    ...
    <h1>Municipality of Tuy Batangas</h1>
        <div class="container">
          <div class="title">Residents Daily Reports</div>
           <div class="content">
            <form method="POST">
...
error_reporting(E_ALL ^ E_WARNING);
   ...
   <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span>
    <center>Please Enter Date!</center>
     ...
     $insertQuery = mysqli_query($con, "INSERT INTO tbl_brgy VALUES ('$ACLE1', '$BAYUDBUD1', '$BOLBOC1', '$DALIMA1', '$DAO1', '$GUINHAWA1', '$LUMBANGAN1', '$LUNTAL1', '$MAGAHIS1', '$MALIBU1', '$MATAYWANAC1', '$PALINCARO1', '$LUNA1', '$BURGOS1', '$RIZAL1', '$RILLO1', '$PUTOL1', '$SABANG1', '$SANJOSE1', '$TALON1', '$TOONG1', '$TUYON1', '$CANCELLED', '$MISSED','$DATE1')");?>
    <script type="text/javascript">
